...
5. **缓存/配准/发布面板** ✅（PR5）——按「不重复造」落地：① 坐标重投影对话框（CRS 注册表 + proj4 transformGeoJSON，纯客户端，结果入新图层）；② XYZ 缓存生成对话框双模式（矢量渲染 generateXyzFromGeoJSON / 底图区域抓取 generateXyzFromRegion，zip 打包下载，进度条）；③ 服务管理/托管目录已接真服务端（GIS 服务管理对话框：连接 services/gis-server、服务注册表、托管本机瓦片目录免上传发布、场景列表，XYZ 托管 URL 可直接粘进「添加数据→XYZ URL」加载）；一键发布已接真服务端（工程→SceneDocument 转换器：geojson 图层内嵌、xyz/wms/wmts/vector-tiles 走 URL 引用、相机取当前视图近似、底图模板写入 basemap 状态；publish-package multipart 发布 + hostedSceneUrl 回显，端到端冒烟通过）；3D Tiles 生成待 sidecar 任务化；处理历史/输出面板已在 PR3 接真数据
